...
Output.fromBufferReader = function(br) {
  ...
  if (obj.satoshis = 0xFFFFFFFFFFFFFFFF)
  {
    ...
    if (size !== 0) {
      ...
      obj.ek = new buffer.Buffer([]);
    }
    size = br.readVarintNum();
    if (size !== 0) {
      obj.ok = br.read(size);
    } else {
      obj.ok = new buffer.Buffer([]);
    }
    size = br.readVarintNum();
    if (size !== 0) {
      obj.sk = br.read(size);
    } else {
      obj.sk = new buffer.Buffer([]);
    }
    obj.bp = {}
    size = br.readVarintNum();
    obj.bp.V =[]
    ...
    if (size !== 0) {
      ...
      obj.bp.A = new buffer.Buffer([]);
    }
    size = br.readVarintNum();
    if (size !== 0) {
      obj.bp.S = br.read(size);
    } else {
      obj.bp.S = new buffer.Buffer([]);
    }
    size = br.readVarintNum();
    if (size !== 0) {
      obj.bp.T1 = br.read(size);
    } else {
      obj.bp.T1 = new buffer.Buffer([]);
    }
    size = br.readVarintNum();
    if (size !== 0) {
      obj.bp.T2 = br.read(size);
    } else {
      obj.bp.T2 = new buffer.Buffer([]);
    }
    size = br.readVarintNum();
    if (size !== 0) {
      obj.bp.taux = br.read(size);
    } else {
      obj.bp.taux = new buffer.Buffer([]);
    }
    size = br.readVarintNum();
    if (size !== 0) {
      obj.bp.mu = br.read(size);
    } else {
      obj.bp.mu = new buffer.Buffer([]);
    }
    size = br.readVarintNum();
    if (size !== 0) {
      obj.bp.a = br.read(size);
    ...
    if (size !== 0) {
      ...
      obj.bp.b = new buffer.Buffer([]);
    }
    size = br.readVarintNum();
    if (size !== 0) {
      obj.bp.t = br.read(size);
    } else {
      obj.bp.t = new buffer.Buffer([]);
    }
  }
  var size = br.readVarintNum();
  if (size !== 0) {
...
Output.prototype.toBufferWriter = function(writer) {
  ...
  if (this.ek.length > 0 || this.ok.length > 0 || this.sk.length > 0) {
