 ...
 * @constructor
 */
function MultiSigInput(input, pubkeys, threshold, signatures, staking) {
...
MultiSigInput.prototype.getSignatures = function(transaction, privateKey, index, sigtype) {
  ...
  _.each(this.publicKeys, function(publicKey) {
    if (publicKey.toString() === privateKey.publicKey.toString()) {
      var s = self.staking ? Script.buildPayToColdStaking(self.staking, self.output.script) : self.output.script;
      results.push(new TransactionSignature({
        publicKey: privateKey.publicKey,
        prevTxId: self.prevTxId,
        outputIndex: self.outputIndex,
        inputIndex: index,
        signature: Sighash.sign(transaction, privateKey, sigtype, index, s),
        sigtype: sigtype
      }));
    }
  });
...
MultiSigInput.prototype.isValidSignature = function(transaction, signature) {
  ...
  var s = this.staking ? Script.buildPayToColdStaking(this.staking, this.output.script) : this.output.script;
