...
class Blsct {
    ...
    CreateTransaction(from, dest, amount, memo, vk, sk, substractFee = true) {
        if (_.isString(dest))
        ...
        for (var out_i in from)
        {
            ...
            if (addedInputs >= amount+(substractFee?0:fee))
                break;
        }
        ...
        if (toXnav)
        {
            destOutput = this.CreateBLSCTOutput(dest, amount-(substractFee?fee:0), memo);
        ...
        {
            destOutput = new transaction.Output({satoshis: amount-(substractFee?fee:0), script: script.fromAddress(dest)})
            let bk = new mcl.Fr();
            bk.setByCSPRNG();
