...
class Blsct {
  ...
  async CreateBLSCTOutput(
    ...
    assert(
      mcl
        .add(mcl.mul(G(), output.gamma), mcl.mul(H(), valueFr))
  ...
  async CreateTransaction(
    ...
    if (!hasMint) {
      for (var out_i in from) {
        ...
        assert(
          key.serializeToHexStr() == mcl.mul(G(), sigk).serializeToHexStr()
        );
    ...
    for (let d in dest) {
      ...
      if (toXnav) {
        destOutput = await this.CreateBLSCTOutput(
          destination.dest,
          destination.amount - (subtractFee ? fee : 0),
          destination.memo,
          tokenId,
          tokenNftId,
          destination.vData,
        ...
        destOutput = new transaction.Output({
          satoshis: destination.amount - (subtractFee ? fee : 0),
          script:
