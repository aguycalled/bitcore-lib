...
class Blsct {
    ...
    async CreateBLSCTOutput (dest, amount, memo="", vdata=new Buffer([])) {
        if (_.isString(dest))
            return await this.CreateBLSCTOutput(new address(dest), amount, memo, vdata);
    ...
    async CreateTransaction(from, dest, amount, memo, vk, sk, subtractFee = true, vData = new Buffer([])) {
        if (_.isString(dest))
            return await this.CreateTransaction(from, new address(dest), amount, memo, vk, sk, subtractFee, vData);

        if (_.isString(vk) || _.isString(sk)) {
            return await this.CreateTransaction(from, dest, amount, memo, mcl.deserializeHexStrToFr(vk), mcl.deserializeHexStrToFr(sk), subtractFee, vData)
        }
        ...
        var keys = [];
        var msgs = [];
        let toXnav = dest instanceof bitcore.Address && dest.isXnav();
        ...
        if (toXnav)
        {
            destOutput = await this.CreateBLSCTOutput(dest, amount-(subtractFee?fee:0), memo, vData);
