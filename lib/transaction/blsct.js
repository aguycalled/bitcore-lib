...
class Blsct {
    ...
    CreateTransaction(from, dest, amount, memo, vk, sk, subtractFee = true) {
        if (_.isString(dest))
            return this.CreateTransaction(from, new address(dest), amount, memo, vk, sk, subtractFee);
        ...
        else
        {
            ...
            destOutput.blstxsig = this.AugmentedSign(bk, destOutput.outhash);
            tx.strdzeel = memo;
        }
    ...
    DeriveMasterKeys(mk) {
        ...
        let childBLSKey = bls().BasicSchemeMPL.derive_child_sk(masterBLSKey, (BIP32_HARDENED_KEY_LIMIT|130)>>>0);
        let transactionBLSKey = bls().BasicSchemeMPL.derive_child_sk(childBLSKey, (BIP32_HARDENED_KEY_LIMIT)>>>0);
        let blindingBLSKey = bls().BasicSchemeMPL.derive_child_sk(childBLSKey, (BIP32_HARDENED_KEY_LIMIT|1)>>>0);
        let viewKey = bls().BasicSchemeMPL.derive_child_sk(transactionBLSKey, (BIP32_HARDENED_KEY_LIMIT)>>>0);
        let spendKey = bls().BasicSchemeMPL.derive_child_sk(transactionBLSKey, (BIP32_HARDENED_KEY_LIMIT|1)>>>0);
