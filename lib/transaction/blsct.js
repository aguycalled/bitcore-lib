...
const {deriveMasterSK, deriveChildSK, deriveChildSKMultiple, pathToIndices} = require("../crypto/bls");
...
let { Init, mcl, bls, maxM, maxMN, maxMessageSize, logN, N, balanceMsg,
      ...
      orderMinusOne, noble} = require('../crypto/blsct');
...
class Blsct {
    ...
    RangeVerify (proofs, vData, nonces, fOnlyRecover, tokenId= new Buffer(new Uint8Array(32)), tokenNftId = -1, fTest= false) {
        let fRecover = false;
        ...
        for (var pi in proofs)
        {
            ...
            if (fRecover)
            {
                ...
                {
                    let fIsMine = mcl.add(mcl.mul(G(),gamma), mcl.mul(H(tokenId, tokenNftId),amount)).isEqual(pd.V[0]);
    ...
    async AugmentedSign(pk, msg) {
        ...
        let m = Buffer.concat([new Buffer(noble.getPublicKey(pk.serialize())), new Buffer(msg)])
    ...
    async CreateBLSCTOutput (dest, amount, memo="", tokenId = new Buffer(new Uint8Array(32)), tokenNftId = -1, vData=new Buffer([]), extraKey = undefined) {
        if (_.isString(dest))
            return await this.CreateBLSCTOutput(new address(dest), amount, memo, tokenId, tokenNftId, vData, extraKey);
        ...
        if (memo.length < maxMessageSize)
            output.memo = memo;
        else {
            output.memo = "____________VDATA_MESSAGE__________";
            let encryptKey = HashG1Element(nonce, 654123);
            vData = Buffer.concat([new Buffer([0xF0]), this.Encrypt(memo, encryptKey)])
        }

        let hashNonce = new mcl.Fr()
        hashNonce.setBigEndianMod(HashG1Element(nonce, 0));

        output.bp = this.RangeProve(values, nonce, output.memo, tokenId, tokenNftId)
    ...
    async CreateTransaction(from, dest, vk, sk, subtractFee = true, tokenId = new Buffer(new Uint8Array(32)),  tokenNftId = -1) {
        if (_.isString(vk) || _.isString(sk)) {
            return await this.CreateTransaction(from, dest, mcl.deserializeHexStrToFr(vk), mcl.deserializeHexStrToFr(sk), subtractFee, tokenId, tokenNftId)
        }
        ...
        for (let i = 0; i < dest.length; i++) {
            ...
            if (dest[i].extraKey)
                dest[i].extraKey = mcl.deserializeHexStrToFr(new Buffer(dest[i].extraKey).toString('hex'));
            if (_.isString(dest[i].dest))
                dest[i].dest = new address(dest[i].dest);
            if (!dest[i].vData)
                dest[i].vData = new Buffer([])
            if (!dest[i].memo)
                dest[i].memo = "";
        }
        ...
        for (var out_i in from)
        {
            ...
            if (!(out.output.tokenId.toString("hex") == tokenId.toString("hex") && out.output.tokenNftId == tokenNftId))
                continue;

            if (!this.IsMine(out.output, vk, sk, out.accIndex[0], out.accIndex[1]))
            {
                console.log('not mine')
                continue;
            }

            this.RecoverBLSCTOutput(out.output, vk, sk, out.accIndex[0], out.accIndex[1])
            ...
            let key = new mcl.G1();
            key.deserialize(out.output.sk.serialize())

            let sigk = new mcl.Fr();
            sigk.deserialize(out.output.sigk.serialize())

            assert(key.serializeToHexStr() == mcl.mul(G(), sigk).serializeToHexStr())

            sigs.push(sig)
            keys.push(key.serialize())
            msgs.push(Buffer.concat([new Buffer(key.serialize()), new Buffer(hash)]))

            noble.utils.setDSTLabel(BasicSchemeMPL_CIPHERSUITE_ID);

            assert(noble.verify(sig, hash, key.serialize()))

            fee = tokenId.toString('hex') == new Buffer(new Uint8Array(32)).toString("hex") ? 200000*(tx.inputs.length+2+dest.length) : 0;

            if (addedInputs >= totalAmount+(subtractFee?0:fee))
                break;
        }
        ...
        for (let d in dest) {
            ...
            let toXnav = destination.dest instanceof address && destination.dest.isXnav();

            if (toXnav)
            {
                destOutput = await this.CreateBLSCTOutput(destination.dest, destination.amount-(subtractFee?fee:0), destination.memo, tokenId, tokenNftId, destination.vData, destination.extraKey);
            ...
            {
                if (tokenId.toString('hex') != new Buffer(new Uint8Array(32)).toString("hex"))
                    throw new Error("Can't send tokens to a NAV address")
                destOutput = new transaction.Output({satoshis: destination.amount-(subtractFee?fee:0), script: destination.dest instanceof script ? destination.dest :  script.fromAddress(destination.dest)})
                let bk = new mcl.Fr();
                bk.setByCSPRNG();
                destOutput.vData = destination.vData;
                destOutput.ek = mcl.mul(G(), bk);
                destOutput.outhash = sha256sha256(destOutput.toBufferWriter().toBuffer());
                destOutput.blstxsig = await this.AugmentedSign(bk, destOutput.outhash);
                if (destination.extraKey) {
                    destOutput.blsextrasig = await this.AugmentedSign(destination.extraKey, destOutput.outhash);
                }
            ...
            if (destination.extraKey)
            {
                ...
                msgs.push(Buffer.concat([new Buffer(this.SkToPubKey(destination.extraKey).serialize()), new Buffer(destOutput.outhash)]))
            }
            sigs.push(destOutput.blstxsig);
            keys.push(destOutput.ek.serialize())
            msgs.push(Buffer.concat([new Buffer(destOutput.ek.serialize()), new Buffer(destOutput.outhash)]))

            tx.addOutput(destOutput);
        }

        tx.addOutput(new transaction.Output({satoshis: fee, script: script.fromHex("6a")}))

        let changeOutput = await this.CreateBLSCTOutput(this.KeysToAddress(viewKey, spendKey).toString(), tx.inputAmount-tx.outputAmount, "Change");

        balk = mcl.sub(balk, changeOutput.bp.V[0])

        sigs.push(changeOutput.blstxsig);
        keys.push(changeOutput.ek.serialize())
        msgs.push(Buffer.concat([new Buffer(changeOutput.ek.serialize()), new Buffer(changeOutput.outhash)]))
    ...
    CombineTransactions(txs) {
        ...
        for (let t in txObjects) {
            ...
            for (let i in tx.inputs) {
                retTx.addInput(tx.inputs[i]);
            }

            for (let i in tx.outputs) {
                if (tx.outputs[i].script.toHex() == "6a" && tx.outputs[i].vData.length == 0) {
            ...
            {
                let sig = new mcl.G2();
                sig.deserialize(tx.vchbalsig);
                balanceSigs.push(sig);
            }

            if (tx.vchtxsig.length)
            {
                let sig = new mcl.G2();
                sig.deserialize(tx.vchtxsig);
                txSigs.push(sig);
            }
        }

        retTx.addOutput(new transaction.Output({satoshis: fee, script: script.fromHex("6a")}))
    ...
    RecoverBLSCTOutput (out, vk, sk, acct, index) {
        ...
        if (!this.RangeVerify([{proof:out.bp,index:0}], vData, [nonce], true, out.tokenId, out.tokenNftId))
            return;
        ...
        if (out.memo == "____________VDATA_MESSAGE__________" && out.vData[0] == 0xF0)
        {
            let encryptKey = HashG1Element(nonce, 654123);
        ...
        if (sk)
            out.sigk = this.RecoverSpendKey(out, vk, sk, acct, index);
    ...
    DeriveMasterKeys(mk) {
        ...
        let childBLSKey = deriveChildSK(masterBLSKey, (BIP32_HARDENED_KEY_LIMIT|130)>>>0);
        let transactionBLSKey = deriveChildSK(childBLSKey, (BIP32_HARDENED_KEY_LIMIT)>>>0);
        let blindingBLSKey = deriveChildSK(childBLSKey, (BIP32_HARDENED_KEY_LIMIT|1)>>>0);
        let viewKey = deriveChildSK(transactionBLSKey, (BIP32_HARDENED_KEY_LIMIT)>>>0);
        let spendKey = deriveChildSK(transactionBLSKey, (BIP32_HARDENED_KEY_LIMIT|1)>>>0);
    ...
    KeysToAddress (vk, sk, network="mainnet") {
        ...
        return address.fromBuffers([new Buffer([0x49, 0x21]), new Buffer(vk.serialize()), new Buffer(sk.serialize())], network, "xnav")
    }

    RangeProve (values, nonce, msg, tokenId= new Buffer(new Uint8Array(32)), tokenNftId = -1, fTest=false) {
        ...
        while (true)
        {
            ...
            for (var i = 0; i < values.length; i++)
            {
                ...
                valuesFr[i] = v;
                proof.V[i] = mcl.add(mcl.mul(G(), gamma[i]), mcl.mul(H(tokenId, tokenNftId), v));
            ...
            try {
                ...
                proof.T1 = mcl.add(mcl.mul(H(tokenId, tokenNftId), t1),mcl.mul(G(), tau1))
                proof.T2 = mcl.add(mcl.mul(H(tokenId, tokenNftId), t2),mcl.mul(G(), tau2))
                ...
                if (fTest)
                {
                    ...
                    assert(mcl.add(mcl.mul(H(tokenId, tokenNftId), mcl.mul(zsq, valuesFr[0])), mcl.mul(G(), mcl.mul(zsq, gamma[0]))).isEqual(
                        mcl.mul(proof.V[0], zsq)
                    ))

                    assert(mcl.add(mcl.add(mcl.mul(H(tokenId, tokenNftId), mcl.mul(zsq, valuesFr[0])), mcl.mul(G(), mcl.mul(zsq, gamma[0]))), mcl.mul(H(tokenId, tokenNftId), d)).isEqual(
                        mcl.add(mcl.mul(proof.V[0], zsq), mcl.mul(H(tokenId, tokenNftId), d))
                    ))

                    assert(mcl.add(mcl.mul(H(tokenId, tokenNftId), mcl.mul(x, t1)), mcl.mul(G(), mcl.mul(x, tau1))).isEqual(
                        mcl.mul(proof.T1, x)
                    ))

                    assert(mcl.add(mcl.mul(H(tokenId, tokenNftId), mcl.mul(xsq, t2)), mcl.mul(G(), mcl.mul(xsq, tau2))).isEqual(
                        mcl.mul(proof.T2, xsq)
                    ))

                    let test_tx = mcl.add(mcl.add(mcl.mul(zsq, gamma[0]), mcl.mul(x, tau1)), mcl.mul(xsq, tau2))
                    const left = mcl.add(mcl.mul(H(tokenId, tokenNftId), test_t), mcl.mul(G(), test_tx))
                    const right = mcl.add(mcl.add(mcl.add(mcl.mul(proof.V[0], zsq), mcl.mul(H(tokenId, tokenNftId), d)), mcl.mul(proof.T1, x)), mcl.mul(proof.T2, xsq))
                ...
                while (nprime > 1)
                {
                    ...
                    // PAPER LINES 23-24
                    proof.L[round] = CrossVectorExponent(nprime, gprime, nprime, hprime, 0, aprime, 0, bprime, nprime, scale, H(tokenId, tokenNftId), mcl.mul(cL, x_ip));
                    proof.R[round] = CrossVectorExponent(nprime, gprime, 0, hprime, nprime, aprime, nprime, bprime, 0, scale, H(tokenId, tokenNftId), mcl.mul(cR, x_ip));
