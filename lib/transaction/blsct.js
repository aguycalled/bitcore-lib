...
class Blsct {
    ...
    async CreateBLSCTOutput (dest, amount, memo="", tokenId = new Buffer(new Uint8Array(32)), tokenNftId = -1, vData=new Buffer([]), extraKey = undefined) {
        ...
        output.bp = this.RangeProve(values, nonce, output.memo)
    ...
    async CreateTransaction(from, dest, vk, sk, subtractFee = true, tokenId = new Buffer(new Uint8Array(32)),  tokenNftId = -1) {
        if (_.isString(vk) || _.isString(sk)) {
            return await this.CreateTransaction(from, dest, amount, memo, mcl.deserializeHexStrToFr(vk), mcl.deserializeHexStrToFr(sk), subtractFee, tokenId, tokenNftId, vData, extraKey)
        }
        ...
        for (var out_i in from)
        {
            ...
            fee = 200000*(tx.inputs.length+2+dest.length);
    ...
    RecoverBLSCTOutput (out, vk, sk, acct, index, tokenId= new Buffer(new Uint8Array(32)), tokenNftId = -1) {
        if (!out.isCt())
            return;

        if (_.isString(vk) || _.isString(sk)){
            return this.RecoverBLSCTOutput(out, mcl.deserializeHexStrToFr(vk), mcl.deserializeHexStrToFr(sk), tokenId, tokenNftId)
        }
        ...
        if (!this.RangeVerify([{proof:out.bp,index:0}], vData, [nonce], true, tokenId, tokenNftId))
            return;
