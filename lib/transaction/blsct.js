...
class Blsct {
    ...
    async CreateBLSCTOutput (dest, amount, memo="", tokenId = new Buffer(new Uint8Array(32)), tokenNftId = -1, vData=new Buffer([])) {
        if (_.isString(dest))
            return await this.CreateBLSCTOutput(new address(dest), amount, memo, tokenId, tokenNftId, vData);
    ...
    async CreateTransaction(from, dest, amount, memo, vk, sk, subtractFee = true, tokenId = new Buffer(new Uint8Array(32)),  tokenNftId = -1, vData = new Buffer([])) {
        if (_.isString(dest))
            return await this.CreateTransaction(from, new address(dest), amount, memo, vk, sk, subtractFee, tokenId, tokenNftId, vData);

        if (_.isString(vk) || _.isString(sk)) {
            return await this.CreateTransaction(from, dest, amount, memo, mcl.deserializeHexStrToFr(vk), mcl.deserializeHexStrToFr(sk), subtractFee, tokenId, tokenNftId, vData)
        }
        ...
        if (toXnav)
        {
            destOutput = await this.CreateBLSCTOutput(dest, amount-(subtractFee?fee:0), memo, tokenId, tokenNftId, vData);
