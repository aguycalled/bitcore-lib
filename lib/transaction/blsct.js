...
class Blsct {
    ...
    async AugmentedSign(pk, msg) {
        ...
        let m = Buffer.concat([noble.getPublicKey(pk.serialize()), msg])
    ...
    async CreateTransaction(from, dest, amount, memo, vk, sk, subtractFee = true) {
        ...
        for (var out_i in from)
        {
            ...
            assert(key.deserializeHexStr() == mcl.mul(G(), sigk).deserializeHexStr())

            sigs.push(sig)
            keys.push(key)
            msgs.push(Buffer.concat([noble.getPublicKey(pk.serialize()), hash]))

            noble.utils.setDSTLabel(BasicSchemeMPL_CIPHERSUITE_ID);

            assert(noble.verify(sig.serialize(), hash, key.serialize()))
        ...
        if (toXnav)
        {
            destOutput = this.CreateBLSCTOutput(dest, amount-(subtractFee?fee:0), memo);
        ...
        msgs.push(Buffer.concat([noble.getPublicKey(pk.serialize()), destOutput.outhash]))
        sigs.push(changeOutput.blstxsig);
        keys.push(changeOutput.ek.serialize())
        msgs.push(Buffer.concat([noble.getPublicKey(pk.serialize()), changeOutput.outhash]))
    ...
    {
        tx.vchbalsig = await this.BasicSign(key, balanceMsg).serialize();
    }
