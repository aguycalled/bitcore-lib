...
let { Init, mcl, bls, maxM, maxMN, maxMessageSize, logN, N, balanceMsg,
      ...
      HadamardFold, VectorPowerSum, subAddressPrefix, ripemd160, sha256 } = require('../crypto/blsct');
...
class Blsct {
    ...
    CreateTransaction(from, dest, amount, memo, vk, sk) {
        ...
        for (var out_i in from)
        {
            ...
            sigs.push(this.AugmentedSign(out.output.sigk, hash))
            keys.push(bls().G1Element.from_bytes(out.output.sk.serialize()))
            msgs.push(hash)
        }
        ...
        tx.vchtxsig = bls().AugSchemeMPL.aggregate(sigs);

        assert(bls().AugSchemeMPL.aggregate_verify(keys, msgs, tx.vchtxsig))

        let balSigKey = mcl.sub(gammasIn, gammasOut);
        tx.vchbalsig = this.BasicSign(balSigKey, balanceMsg);
    ...
    GetHashId (out, vk) {
        ...
        let dh = mcl.add(out.sk, mcl.inv(mcl.mul(G(), hash_t)));

        return ripemd160(sha256(dh));
    }
