...
class Blsct {
    ...
    async CreateTransaction(from, dest, amount, memo, vk, sk, subtractFee = true) {
        ...
        for (var out_i in from)
        {
            ...
            msgs.push(Buffer.concat([key.serialize(), hash]))
        ...
        msgs.push(Buffer.concat([destOutput.ek.serialize(), destOutput.outhash]))
        sigs.push(changeOutput.blstxsig);
        keys.push(changeOutput.ek.serialize())
        msgs.push(Buffer.concat([changeOutput.ek.serialize(), changeOutput.outhash]))
