...
const {Output, UnspentOutput} = Transaction;
...
class Blsct {
  ...
  async BalanceSigVerify(pk, sig) {
    if (_.isString(pk)) {
      return this.BalanceSigVerify(mcl.deserializeHexStrToG1(pk), sig);
    }

    let s = await noble.verify(sig, balanceMsg, pk.serialize());

    return s;
  }

  async AugmentedVerify(pk, msg, sig) {
    if (_.isString(pk)) {
      return this.AugmentedVerify(mcl.deserializeHexStrToG1(pk), msg, sig);
    }

    noble.utils.setDSTLabel(AugSchemeMPL_CIPHERSUITE_ID);

    let m = Buffer.concat([
      new Buffer(pk.serialize()),
      new Buffer(msg),
    ]);
    let s = await noble.verify(sig, m, pk.serialize());
  ...
  H(tokenId = new Buffer(new Uint8Array(32)),
    tokenNftId = -1) {
    return H(tokenId, tokenNftId);
  }
  ...
  async CreateTransaction(
    ...
    for (let d in dest) {
      ...
      if (toXnav) {
        destOutput = await this.CreateBLSCTOutput(
            ...
            destination.tokenNftId,
            destination.vData,
            destination.extraKey
        );
        if (destination.tokenNftId == -1) {
        ...
        destOutput.ok = new mcl.G1();
        destOutput.sk = new mcl.G1();
    ...
    tx.addOutput(
        new Output({ satoshis: fee, script: script.fromHex("6a") })
    );

    if (!hasMint && addedInputs - totalAmount - (!subtractFee ? aggFee + fee : 0) > 0) {
  ...
  RecoverBLSCTOutput(out, vk, sk, acct, index) {
    ...
    out.amount = out.satoshis;
    out.gamma = new mcl.Fr();

    if (out.isNft() && !out.isCt()) {
      if (sk) out.sigk = this.RecoverSpendKey(out, vk, sk, acct, index);
      return out;
    } else if (!out.isCt()) return;
