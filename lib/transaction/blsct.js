...
class Blsct {
    ...
    async CreateBLSCTOutput (dest, amount, memo="", tokenId = new Buffer(new Uint8Array(32)), tokenNftId = -1, vData=new Buffer([]), extraKey = undefined) {
        ...
        if (extraKey) {
            let extraSig = await this.AugmentedSign(extraKey, hash);
            output.blstxsig = noble.aggregateSignatures([output.blstxsig, extraSig]);
        }
        output.outhash = hash;
    ...
    async CreateTransaction(from, dest, amount, memo, vk, sk, subtractFee = true, tokenId = new Buffer(new Uint8Array(32)),  tokenNftId = -1, vData = new Buffer([]), extraKey = undefined) {
        if (_.isString(dest))
            return await this.CreateTransaction(from, new address(dest), amount, memo, vk, sk, subtractFee, tokenId, tokenNftId, vData, extraKey);
        ...
        if (extraKey)
        {
            extraKey = mcl.deserializeHexStrToFr(new Buffer(extraKey).toString('hex'))
        }
        ...
        var keys = [];
        var msgs = [];
        let toXnav = dest instanceof address && dest.isXnav();
        ...
        if (toXnav)
        {
            destOutput = await this.CreateBLSCTOutput(dest, amount-(subtractFee?fee:0), memo, tokenId, tokenNftId, vData, extraKey);
        ...
        {
            destOutput = new transaction.Output({satoshis: amount-(subtractFee?fee:0), script: dest instanceof script ? dest :  script.fromAddress(dest)})
            let bk = new mcl.Fr();
            bk.setByCSPRNG();
        ...
        if (extraKey)
        {
            ...
            keys.push(this.SkToPubKey(extraKey).serialize())
            msgs.push(Buffer.concat([new Buffer(this.SkToPubKey(extraKey).serialize()), new Buffer(destOutput.outhash)]))
        }
        sigs.push(destOutput.blstxsig);
