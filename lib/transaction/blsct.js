...
class Blsct {
    ...
    async CreateTransaction(from, dest, amount, memo, vk, sk, subtractFee = true) {
        if (_.isString(dest))
            return await this.CreateTransaction(from, new address(dest), amount, memo, vk, sk, subtractFee);

        if (_.isString(vk) || _.isString(sk)) {
            return await this.CreateTransaction(from, dest, amount, memo, mcl.deserializeHexStrToFr(vk), mcl.deserializeHexStrToFr(sk))
        }
        ...
        if (toXnav)
        {
            destOutput = await this.CreateBLSCTOutput(dest, amount-(subtractFee?fee:0), memo);
        ...
        {
            destOutput = new transaction.Output({satoshis: amount-(subtractFee?fee:0), script: script.fromAddress(dest)})
            let bk = new mcl.Fr();
            bk.setByCSPRNG();
