...
Transaction.prototype.fromObject = function fromObject(arg) {
  ...
  _.each(transaction.inputs, function(input) {
    ...
    } else if ((script.isColdStakingOut() || script.isScriptHashOut()) && input.publicKeys && input.threshold) {
...
Transaction.prototype.from = function(utxo, pubkeys, threshold, staking) {
  console.log("from got: "+[utxo, pubkeys, threshold, staking])
  if (_.isArray(utxo)) {
    var self = this;
  ...
  if (pubkeys && threshold && staking) {
    console.log("_fromMultisigUtxo => " [utxo, pubkeys, threshold, staking]);
...
Transaction.prototype._fromMultisigUtxo = function(utxo, pubkeys, threshold, staking) {
  ...
  if (utxo.script.isMultisigOut()) {
    ...
    clazz = MultiSigScriptHashInputColdStaking;

    console.log("MultiSigScriptHashInputColdStaking => " [{
      output: new Output({
        script: utxo.script,
        satoshis: utxo.satoshis
      }),
