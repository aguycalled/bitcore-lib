...
Transaction.prototype.toBufferWriter = function (writer) {
  ...
  if (this.version & 0x10 || this.version & 0x20) {
    writer.writeVarintNum(Buffer.from(new Uint8Array(this.vchbalsig)).length);
    writer.write(Buffer.from(new Uint8Array(this.vchbalsig)));
  }
  if (this.version & 0x10) {
    writer.writeVarintNum(Buffer.from(new Uint8Array(this.vchtxsig)).length);
    writer.write(Buffer.from(new Uint8Array(this.vchtxsig)));
  }
  return writer;
};
...
Transaction.prototype.toObject = Transaction.prototype.toJSON =
    function toObject() {
      ...
      var obj = {
        ...
        vchbalsig: Buffer.from(new Uint8Array(this.vchbalsig)),
        vchtxsig: Buffer.from(new Uint8Array(this.vchtxsig)),
      };
...
Transaction.prototype.fromObject = function fromObject(arg) {
  ...
  _.each(transaction.inputs, function (input) {
    ...
    if (
        ...
        script.isColdStakingOutP2PKH() ||
        script.isColdStakingV2Out()
    ) {
      txin = new Input.PublicKeyHash(input);
    } else if (
        script.isColdStakingOut() &&
        input.publicKeys &&
        input.threshold
    ) {
  ...
  this.vchbalsig = Buffer.from(transaction.vchbalsig);
  this.vchtxsig = Buffer.from(transaction.vchtxsig);
...
Transaction.prototype.isCoinStake = function () {
  return (
      ...
      !this.inputs[0].IsNull() &&
      this.outputs.length >= 2 &&
      this.outputs[0].satoshis == 0 &&
      this.outputs[0].script.chunks.length == 0
  );
};
 ...
 */
Transaction.prototype.from = function (
    utxo,
    pubkeys,
    threshold,
    staking,
...
Transaction.prototype._fromNonP2SH = function (utxo) {
  ...
  if (
      ...
      utxo.script.isColdStakingOutP2PKH() ||
      utxo.script.isColdStakingV2Out()
  ) {
...
Transaction.prototype._fromMultisigUtxo = function (
    utxo,
    pubkeys,
    threshold,
    staking
...
Transaction.prototype.addInput = function (input, outputScript, satoshis) {
  ...
  if (
      (input.output.ek && !input.output.ek.isZero()) ||
      (input.output.ok && !input.output.ok.isZero()) ||
      (input.output.sk && !input.output.sk.isZero())
  ) {
    this.version |= 0x10;
  }
...
Transaction.prototype.change = function (address) {
  ...
  if (address instanceof Script) this._changeScript = address;
  else this._changeScript = Script.fromAddress(address);
  this._updateChangeOutput();
  return this;
};
...
Transaction.prototype.addOutput = function (output) {
  ...
  if (
      (output.ek && !output.ek.isZero()) ||
      (output.ok && !output.ok.isZero()) ||
      (output.sk && !output.sk.isZero())
  ) {
    this.version |= 0x20;
  }
...
Transaction.prototype._getInputAmount = function () {
  if (_.isUndefined(this._inputAmount)) {
    ...
    _.each(this.inputs, function (input) {
      ...
      self._inputAmount += input.output.isCt()
          ? input.output.amount
