 ...
 */
Transaction.prototype.from = function(utxo, pubkeys, threshold, staking) {
  if (_.isArray(utxo)) {
    var self = this;
...
Transaction.prototype.addOutput = function(output) {
  ...
  if ((output.ek && output.ek.length > 0) || (output.ok && output.ok.length > 0) || (output.sk && output.sk.length > 0)) {
    this.version |= 0x20;
  }
...
Transaction.prototype._getOutputAmount = function() {
  if (_.isUndefined(this._outputAmount)) {
    ...
    _.each(this.outputs, function(output) {
      self._outputAmount += output.isCt() ? output.value : output.satoshis;
    });
  }
