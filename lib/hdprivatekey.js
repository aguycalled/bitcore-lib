...
HDPrivateKey.prototype.deriveChild = function(arg, hardened) {
  if (_.isNumber(arg)) {
    ...
    if (arg.substr(0,2) == '0x') {
      var hash = Hash.sha256(BufferUtil.hexToBuffer(arg.substr(2)));
      return this._deriveFromBuffer(hash);
    } else {
...
HDPrivateKey.prototype._deriveFromBuffer = function(indexBuffer) {
  console.log(indexBuffer.length)
  if (indexBuffer.length != 32)
  ...
  for (var i = 0; i < 8; i++)
  {
    derived = derived._deriveFromBufferPart(indexBuffer.slice(i*4, (i*4)+4));
  }

  return derived;
};

HDPrivateKey.prototype._deriveFromBufferPart = function(indexBuffer) {
  ...
  if (!PrivateKey.isValid(privateKey)) {
    // Index at this point is already hardened, we can pass null as the hardened arg
    return this._deriveFromBufferPart(BufferUtil.integerAsBuffer(BufferUtil.integerFromBuffer(indexBuffer) + 1));
  }
  ...
  var derived = new HDPrivateKey({
    ...
    depth: this.depth + 1,
    parentFingerPrint: this.fingerPrint,
    childIndex: BufferUtil.integerFromBuffer(indexBuffer),
    chainCode: chainCode,
    privateKey: privateKey
  });
